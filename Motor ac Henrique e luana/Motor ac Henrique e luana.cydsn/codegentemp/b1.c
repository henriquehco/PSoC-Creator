...
#include "b1.h"
...
#if defined(b1__PC)
    #define b1_SetP4PinDriveMode(shift, mode)  \
    do { \
        b1_PC =   (b1_PC & \
                                (uint32)(~(uint32)(b1_DRIVE_MODE_IND_MASK << \
                                (b1_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (b1_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define b1_SetP4PinDriveMode(shift, mode)  \
        do { \
            b1_USBIO_CTRL_REG = (b1_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(b1_DRIVE_MODE_IND_MASK << \
                                    (b1_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (b1_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		b1_SetP4PinDriveMode(b1__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(b1_DR & (uint8)(~b1_MASK));
    drVal = (drVal | ((uint8)(value << b1_SHIFT) & b1_MASK));
    b1_DR = (uint32)drVal;
}
...
uint8 b1_Read(void)
{
    return (uint8)((b1_PS & b1_MASK) >> b1_SHIFT);
}
...
{
    return (uint8)((b1_DR & b1_MASK) >> b1_SHIFT);
}
...
void b1_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  b1_INTCFG & (uint32)(~(uint32)position);
    b1_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(b1_INTSTAT & b1_MASK);
	b1_INTSTAT = maskedStatus;
    return maskedStatus >> b1_SHIFT;
}
