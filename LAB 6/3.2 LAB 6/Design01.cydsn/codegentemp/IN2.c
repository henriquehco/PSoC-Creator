...
#include "IN2.h"
...
#if defined(IN2__PC)
    #define IN2_SetP4PinDriveMode(shift, mode)  \
    do { \
        IN2_PC =   (IN2_PC & \
                                (uint32)(~(uint32)(IN2_DRIVE_MODE_IND_MASK << \
                                (IN2_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (IN2_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define IN2_SetP4PinDriveMode(shift, mode)  \
        do { \
            IN2_USBIO_CTRL_REG = (IN2_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(IN2_DRIVE_MODE_IND_MASK << \
                                    (IN2_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (IN2_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		IN2_SetP4PinDriveMode(IN2__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(IN2_DR & (uint8)(~IN2_MASK));
    drVal = (drVal | ((uint8)(value << IN2_SHIFT) & IN2_MASK));
    IN2_DR = (uint32)drVal;
}
...
uint8 IN2_Read(void)
{
    return (uint8)((IN2_PS & IN2_MASK) >> IN2_SHIFT);
}
...
{
    return (uint8)((IN2_DR & IN2_MASK) >> IN2_SHIFT);
}
...
void IN2_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  IN2_INTCFG & (uint32)(~(uint32)position);
    IN2_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(IN2_INTSTAT & IN2_MASK);
	IN2_INTSTAT = maskedStatus;
    return maskedStatus >> IN2_SHIFT;
}
